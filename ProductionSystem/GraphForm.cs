...
public partial class GraphForm : Form
{
	...
	private void DrawFactNodes(Graphics g)
	{
		foreach (var fact in _kb.Facts.Values)
		{
			...
			using (var brush = new SolidBrush(backColor)) using (var pen = new Pen(Color.Black, usedBackward ? 2f : 1f))
			{
	...
	private void DrawRuleEdges(Graphics g)
	{
		foreach (var rule in _kb.Rules)
		{
			...
			using (var brush = new SolidBrush(fillColor)) using (var penNode = new Pen(borderColor, borderWidth))
			{
